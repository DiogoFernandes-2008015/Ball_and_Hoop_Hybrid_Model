...
def hybrid_dc_motor_model_jax(t, w, args):
    ...
    theta1, theta3,fc,fs,vs, params_nn, u_interp = args
    ...
    return theta1 * w + theta3 * u + (fc+(fs-fc)*jnp.exp(-jnp.abs(w/vs)**2))*jnp.sign(w)+nn_output[0]
...
nn_layer_sizes = [1, 64,64, 1]  # 2 inputs (w, u), 2 hidden layers of 10, 1 output
solver = Dopri5()
...
initial_guess_np = np.concatenate([
    np.array([initial_theta1, initial_theta3, initial_fc, initial_fs, initial_vs]),
...
def objective_jax_nn(decision_vars):
    ...
    fs = decision_vars[3]
    vs = decision_vars[4]
    params_nn = params_nn_struct(decision_vars[5:len_nn_params+5])

    w_initial_shots = decision_vars[len_nn_params+5:]

    def simulate_shot(t_shot, w0):
        saveat = SaveAt(ts=t_shot)
        args = (theta1, theta3,fc,fs,vs, params_nn, u_interpolation)
...
max_iterations = 100 # Increased iterations for the more complex model
...
sc.io.savemat('resultados_strib_hib_nn.mat',results)
